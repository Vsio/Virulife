...
class CFB {
    ...
    public CFB(String _newInputText, String _newKey, String _newOutputText)
    /* constructs CFB instance */
    {
	...
	/* deciphers with CFB method */
	{
		...
		for (i=0;i!=blockLength;i++) {
			blockPlain += xorBit(blockCipher[i],inputBit[j]);
			blockTemp += inputBit[j];
			j += 1;
		}

		output += blockPlain;
		blockCipher = decryptionBlock(blockTemp);
		blockPlain = "";
		blockTemp = "";
	...
	{	
		if (_index == 0) {
			try {				
				...
				Console.WriteLine("\n> CFB enciphering result has been saved into output_encipher.txt");
			} catch (Exception) {
			...
			try {
				...
				Console.WriteLine("\n> CFB deciphering result has been saved into output_encipher.txt");
			} catch (Exception) {
				Console.WriteLine("> File not found.");
			}
		}
